...
<template>
  <div class="h-dvh flex flex-col bg-base-100 text-base-content">
    ...
    <DuNavbar class="sticky top-0 z-50 shadow-none! bg-base-100/80 backdrop-blur border-b-2 border-base-300 shrink-0">
      ...
      <template #end>
        <div class="flex flex-row-reverse items-center gap-1.5 px-2">
          <!-- Active repo link -->
          <a
            :href="repoUrl"
            target="_blank"
            rel="noopener"
            :title="`Open on ${preference === 'gitlab' ? 'GitLab' : preference === 'github' ? 'GitHub' : 'npm'}`"
          ...
          >
            ...
            <span class="hidden sm:inline">{{ preference === 'gitlab' ? 'GitLab' : preference === 'npm' ? 'npm' : 'GitHub' }}</span>
          </a>

          <!-- Switch source button -->
          <div
            class="tooltip tooltip-left"
            :data-tip="preference === 'gitlab' ? 'Switch to GitHub' : preference === 'github' ? 'Switch to npm' : 'Switch to GitLab'"
          >
            <button
              :aria-label="preference === 'gitlab' ? 'Switch to GitHub' : preference === 'github' ? 'Switch to npm' : 'Switch to GitLab'"
              ...
              @click="set(preference === 'gitlab' ? 'github' : preference === 'github' ? 'npm' : 'gitlab')"
            >
